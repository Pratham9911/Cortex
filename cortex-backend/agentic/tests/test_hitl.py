...
from agentic.checkpointer import init_checkpointer, close_checkpointer, delete_checkpoint
...
async def test_hitl_approval_flow():
    ...
    final_answer = ""
    async for update in workflow.astream(Command(resume="yes"), config=config, stream_mode="updates"):
        print(f"Update received: {list(update.keys())}")
        for node_name, node_update in update.items():
