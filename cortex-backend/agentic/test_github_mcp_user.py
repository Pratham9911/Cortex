...
async def run_user_github_mcp_test(user_id: int, prompt: str):
    db = SessionLocal()
    try:
        # 1. Obtain MCP Client configured with current Cortex user's GitHub OAuth token
        try:
            client = await get_github_mcp_client_for_user(user_id=user_id, db=db)
        except ValueError:
            # ── GitHub not connected: let the agent explain it naturally ──────────
            llm = get_llm()
        ...
        # 3. Filter discovered tools using the Cortex JSON tool registry
        selected_tools, tool_config = filter_github_mcp_tools(
            discovered_tools=all_discovered_tools,
            log_summary=True,
        )

        # 4. Context & Token Schema reduction diagnostic
        schema_chars_all = 0
        ...
        # 5. LangGraph integration with ONLY selected enabled tools
        llm = get_llm()
        ...
        async def chat_node(state: ChatState):
            ...
            for idx, msg in enumerate(messages, 1):
                ...
                # Content preview (trim long content to 400 chars)
                raw_content = msg.content if isinstance(msg.content, str) else str(msg.content)
                content_preview = raw_content[:2400] + ("..." if len(raw_content) > 400 else "")

                print(f"\n  [{idx}] {role}")
                if content_preview.strip():
                    print(f"       Content: {content_preview}")

                # Tool calls (AI messages calling tools)
        ...
        # 6. Execute prompt
        print("=" * 70)
        print(f"[TEST PROMPT FOR USER {user_id}]: {prompt}")
        print("=" * 70)
...
if __name__ == "__main__":
    ...
    asyncio.run(run_user_github_mcp_test(user_id=user_id_arg, prompt=prompt_arg))
