...
type MemberDetails = TeamMember & { joined_at?: string; teams: Array<{ team_id: number; name: string; added_at?: string }> }
...
export function TasksTab({ isDark, members = [], teamId, canManage = false }: { isDark: boolean; members?: TeamMember[]; teamId: number; canManage?: boolean }) {
  ...
  const reorderDraftSubtasks = (fromIndex: number, toIndex: number) => setDraftSubtasks((current) => {
  ...
  const createTask = async () => {
    ...
    if (!newTaskStatus || !draftTitle.trim() || !draftDueDate || (!editingTaskId && draftDueDate < today) || normalizedSubtasks.length === 0 || normalizedSubtasks.some((title) => !title)) return
    try {
      const { token, projectId } = getTaskContext()
      const payload = { title: draftTitle.trim(), description: draftDescription.trim(), status: newTaskStatus, due_date: draftDueDate, priority: draftPriority, assignee_ids: draftAssignees, tags: draftTags, subtasks: normalizedSubtasks.map((title, index) => ({ title, position: index + 1 })) }
  ...
  const openMemberDetails = async (member: TeamMember) => {
    ...
    setMemberDetails({ ...member, teams: [] })
    setMemberDetailsLoading(true)
    setMemberDetailsError("")
    try {
      const { token, projectId } = getTaskContext()
      const response = await fetch(`${apiUrl}/projects/${projectId}/members/${member.user_id}/details`, { headers: { Authorization: `Bearer ${token}` } })
      const data = await response.json().catch(() => ({}))
      if (!response.ok) throw new Error(data?.detail || "Could not load member details")
      setMemberDetails(data)
    } catch (error) { setMemberDetailsError(error instanceof Error ? error.message : "Could not load member details") } finally { setMemberDetailsLoading(false) }
  }
  ...
  return <section className="min-w-0 space-y-5">{taskError && <p className="rounded-md border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-sm text-rose-600 dark:text-rose-300">{taskError}</p>}<div className="flex flex-wrap items-center justify-between gap-3 border-b pb-4 dark:border-zinc-800"><div className="flex items-center gap-1 sm:gap-2"><Button variant="ghost" size="sm" className={cn("h-9 rounded-none border-b-2 px-2 font-semibold", isDark ? "border-white text-white hover:bg-transparent" : "border-black text-black hover:bg-transparent")}><LayoutGrid className="size-4" />Board view</Button><Button variant="ghost" size="sm" className={cn("h-9 rounded-none px-2", isDark ? "text-zinc-500 hover:bg-transparent hover:text-zinc-200" : "text-slate-500 hover:bg-transparent hover:text-slate-900")}><List className="size-4" />List view</Button></div><div className="flex flex-wrap items-center gap-2"><div className="relative"><Button variant="outline" size="sm" onClick={() => setFilterOpen((open) => !open)} className={cn("rounded-md", filterOpen && (isDark ? "border-white bg-white text-black" : "border-black bg-black text-white"))}><SlidersHorizontal className="size-3.5" />Filter <ChevronDown className="size-3.5" /></Button>{filterOpen && <div className={cn("absolute right-0 z-20 mt-2 w-64 border p-3 shadow-lg", isDark ? "border-zinc-700 bg-[#191919]" : "border-slate-200 bg-white")}><p className="text-xs font-semibold">Show priority</p><Select value={priorityFilter} onValueChange={(value) => setPriorityFilter(value as Priority | "ALL")}><SelectTrigger className="mt-2 w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ALL">All priorities</SelectItem><SelectItem value="HIGH">High priority</SelectItem><SelectItem value="MEDIUM">Medium priority</SelectItem><SelectItem value="LOW">Low priority</SelectItem></SelectContent></Select><p className="mt-4 text-xs font-semibold">Sort tasks</p><Select value={sortBy} onValueChange={(value) => setSortBy(value as "DUE_DATE" | "PRIORITY")}><SelectTrigger className="mt-2 w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="DUE_DATE">Due date, nearest first</SelectItem><SelectItem value="PRIORITY">Priority, highest first</SelectItem></SelectContent></Select></div>}</div>{canManage && <Button size="sm" onClick={() => setNewTaskStatus("TODO")} className={cn("rounded-md", isDark ? "bg-white text-black hover:bg-zinc-200" : "bg-black text-white hover:bg-zinc-800")}><Plus className="size-4" />New task</Button>}</div></div>
    ...
    <Dialog open={memberDetailsOpen} onOpenChange={(open) => { setMemberDetailsOpen(open); if (!open) { setMemberDetails(null); setMemberDetailsError("") } }}>
      <DialogContent className={cn("sm:max-w-lg", isDark ? "border-zinc-800 bg-[#151515] text-white" : "border-slate-200 bg-white")}>
        <DialogHeader><DialogTitle>Member details</DialogTitle><DialogDescription className={isDark ? "text-zinc-400" : "text-slate-500"}>Project role, team memberships, and joined date.</DialogDescription></DialogHeader>
        {memberDetailsLoading && !memberDetails ? <div className="flex min-h-48 items-center justify-center"><span className="size-6 animate-spin rounded-full border-2 border-zinc-400 border-t-transparent" /></div> : memberDetails ? <div className="space-y-4"><div className={cn("rounded-xl border p-4", isDark ? "border-zinc-700 bg-zinc-900/40" : "border-slate-200 bg-slate-50")}><div className="flex items-center gap-3"><span className={cn("flex size-12 items-center justify-center overflow-hidden rounded-full border-2 text-xs font-bold", isDark ? "border-white bg-zinc-100 text-black" : "border-black bg-white text-black")}>{memberDetails.avatar_url ? <img src={memberDetails.avatar_url} alt={memberDetails.name} className="size-full object-cover" /> : initials(memberDetails.name)}</span><div className="min-w-0"><div className="flex items-center gap-2"><p className="truncate text-base font-bold">{memberDetails.name}</p><span className={cn("shrink-0 px-2 py-0.5 text-[10px] font-bold", isDark ? "bg-zinc-700 text-zinc-200" : "bg-slate-200 text-slate-700")}>{memberDetails.user_id}</span></div><p className="truncate text-xs text-zinc-500">{memberDetails.email || "No email available"}</p></div></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><div><p className="text-[10px] font-bold uppercase tracking-wide text-zinc-500">Role</p><p className="mt-1 text-sm font-semibold capitalize">{memberDetails.role || "member"}</p></div><div><p className="text-[10px] font-bold uppercase tracking-wide text-zinc-500">Joined</p><p className="mt-1 text-sm font-semibold">{memberDetails.joined_at ? formatDueDate(memberDetails.joined_at.slice(0, 10)) : "Not available"}</p></div></div></div><div><p className="mb-2 text-xs font-bold uppercase tracking-wide text-zinc-500">Teams</p><div className="flex flex-wrap gap-2">{memberDetails.teams.map((team) => <span key={team.team_id} className={cn("px-3 py-1 text-xs font-semibold", isDark ? "bg-zinc-800 text-zinc-300" : "bg-slate-100 text-slate-700")}>{team.name}</span>)}{memberDetails.teams.length === 0 && <span className="text-sm text-zinc-500">No teams found.</span>}</div></div>{memberDetailsError && <p className="text-sm text-rose-500">{memberDetailsError}</p>}</div> : <p className="text-sm text-rose-500">{memberDetailsError || "Could not load member details."}</p>}
      </DialogContent>
    </Dialog>
    <Dialog open={!!assigneeListTaskId} onOpenChange={(open) => !open && setAssigneeListTaskId(null)}>
      <DialogContent className={cn("sm:max-w-sm", isDark ? "border-zinc-800 bg-[#151515] text-white" : "border-slate-200 bg-white")}>
        ...
        <div className="max-h-80 space-y-1 overflow-y-auto">{(tasks.find((task) => task.id === assigneeListTaskId)?.assignees || []).map((id) => { const person = people.find((member) => member.user_id === id) || { user_id: id, name: `Member ${id}` }; return <button key={person.user_id} type="button" onClick={() => { setAssigneeListTaskId(null); openMemberDetails(person) }} className={cn("flex w-full items-center gap-3 rounded-md px-2 py-2 text-left", isDark ? "hover:bg-zinc-800" : "hover:bg-slate-100")}><span className={cn("flex size-8 items-center justify-center overflow-hidden rounded-full border text-[10px] font-bold", isDark ? "border-white bg-zinc-100 text-black" : "border-black bg-white text-black")}>{person.avatar_url ? <img src={person.avatar_url} alt="" className="size-full object-cover" /> : initials(person.name)}</span><span className="truncate text-sm font-medium">{person.name}</span></button> })}</div>
      </DialogContent>
    </Dialog>
      ...
      <DialogContent className={cn("max-h-[90vh] overflow-y-auto rounded-xl border p-0 sm:max-w-2xl", isDark ? "border-zinc-800 bg-[#151515] text-white" : "border-slate-200 bg-white")}>
        ...
        <div className="space-y-5 px-6 py-5">
          ...
          <div>
            ...
            <div className="flex gap-2"><Input value={draftSubtask} maxLength={80} onChange={(event) => setDraftSubtask(event.target.value)} placeholder="Add a subtask at the end" onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); appendDraftSubtask() } }} className={isDark ? "border-zinc-700 bg-zinc-900" : ""} /><Button type="button" variant="outline" disabled={!draftSubtask.trim()} onClick={appendDraftSubtask}>Add</Button></div>
            <div className="mt-3 space-y-2">
              {draftSubtasks.map((subtask, index) => <div key={index} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (draggedDraftSubtaskIndex !== null) reorderDraftSubtasks(draggedDraftSubtaskIndex, index); setDraggedDraftSubtaskIndex(null) }} className={cn("flex items-center gap-2 rounded-lg border p-2 transition-colors", draggedDraftSubtaskIndex === index && "border-dashed opacity-60", isDark ? "border-zinc-800 bg-zinc-900/70" : "border-slate-200 bg-slate-50")}><span draggable title="Drag to reorder" onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; setDraggedDraftSubtaskIndex(index) }} onDragEnd={() => setDraggedDraftSubtaskIndex(null)} className="flex size-6 shrink-0 cursor-grab items-center justify-center rounded text-zinc-500 active:cursor-grabbing"><GripVertical className="size-4" /></span><span className="sr-only">Subtask {index + 1}</span><Input value={subtask} maxLength={80} aria-label={`Subtask ${index + 1}`} onChange={(event) => updateDraftSubtask(index, event.target.value)} placeholder="Describe this subtask" className={cn("h-8 border-0 bg-transparent px-1 shadow-none focus-visible:ring-0", isDark ? "text-white placeholder:text-zinc-600" : "text-slate-900 placeholder:text-slate-400")} /><div className="flex shrink-0 items-center"><Button type="button" variant="ghost" size="icon" title="Insert subtask below" onClick={() => insertDraftSubtask(index + 1)} className="size-7"><Plus className="size-3.5" /></Button><Button type="button" variant="ghost" size="icon" title="Remove subtask" onClick={() => removeDraftSubtask(index)} className="size-7 text-rose-500 hover:text-rose-600"><Trash2 className="size-3.5" /></Button></div></div>)}
            </div>
